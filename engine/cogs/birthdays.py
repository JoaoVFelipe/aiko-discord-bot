...
class Birthdays(commands.Cog):
    def __init__(self, bot: commands.Bot, store: BirthdaysStore):
        self.store = store
        self.bot = bot
    ...
    async def bday_add(self, ctx, ddmm: str):
        ...
        await self.store.upsert_birthday_user(str(ctx.guild.id), str(ctx.author.id), ddmm, name=member.display_name)
    ...
    async def bday_remove(self, ctx, member: discord.Member):
        ...
        ok = await self.store.remove_birthday_user(str(ctx.guild.id), str(member.id))
    ...
    async def bday_list(self, ctx):
        ...
        users = await self.store.list_birthday_users(str(ctx.guild.id))
        if not users:
    ...
    async def set_bday_channel(self, ctx: commands.Context, channel: discord.TextChannel | None):
        ...
        await self.store.update_guild_cfg(str(ctx.guild.id), birthday_channel_id=str(cid) if cid else None)
    ...
    async def set_bday_time(self, ctx: commands.Context, time_str: str):
        ...
        await self.store.update_guild_cfg(
            str(ctx.guild.id),
            birthday_hour=hour,
...
async def setup(bot: commands.Bot):
    ...
    await bot.add_cog(Birthdays(bot, store))